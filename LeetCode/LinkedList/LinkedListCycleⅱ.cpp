 ...
 * also:   @  a = c+ (n− 1)(b+ c)
 * Fp and Sp will meet at the first round (for Sp),so n = 1, as a result:
 *         @  a = c
 * At this time " Ano_point " start movement with the same speed as Sp, in this case, " Ano_point " will meet " Slow_point " in entrance.
 */           
    ...
    ListNode *detectCycle(ListNode *head) {
        ...
        for(int i = 0; head != NULL && head->next !=NULL ; ++i){
            if(Fast_point->next!= 0 && Fast_point->next->next != 0){
                ...
                break; //Linked List have not cycle.
